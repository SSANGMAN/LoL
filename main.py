...
if algorithm == 'RandomForest':
    def RandomForestEvaluation(params):
        ...
        gc.collect()
        print("Bayesian Optimization RandomForest Model Mean Accuracy".format(
            score_mean/k))
    ...
    def XGBEvaluation(params):
        ...
        for tr_idx, val_idx in kf.split(X_train, y_train):
            ...
            clf.fit(X_tr, y_tr, eval_set=[
                    (X_tr, y_tr), (X_vl, y_vl)], early_stopping_rounds=50, verbose=False)
            pred = clf.predict(X_vl)
        ...
        gc.collect()
        print("Bayesian Optimization XGBoost Model Mean Accuracy: {}".format(
            score_mean/k))
...
if algorithm == 'RandomForest':
    best = fmin(fn=RandomForestEvalation, space=space,
                algo=tpe.suggest, max_eval=optimization_iter)
elif algorithm == 'XGB':
    best = fmin(fn=XGBEvaluation, space=space,
                algo=tpe.suggest, max_evals=optimization_iter)
...
print("HyperParameter Tuning XGBClassifier Accuracy Score: ",
      accuracy_score(pred, y_test))

# Save Model
joblib.dump(clf, os.path.join(model_output_path, 'XGB_{}_{}'.format(
    round(accuracy_score(pred, y_test), 3), minute)))
